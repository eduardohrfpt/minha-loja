...
// Mesmo conteúdo legal da versão atual, palavra por palavra -- só o layout muda aqui. Textos
// jurídicos não são "copywriting" pra reescrever; são compromissos com o cliente.
const ABAS = [
  {
    slug: 'termos-de-uso',
...
function V2PoliticasPage() {
  const { aba } = useParams()
  const abaAtiva = ABAS.find((item) => item.slug === aba)

  if (!abaAtiva) {
    return <Navigate to={`/v2/politicas/${ABAS[0].slug}`} replace />
  }
  ...
  return (
    <section className="v2-pagina-institucional">
      <div className="v2-container">
        ...
        <nav className="v2-institucional-abas">
          {ABAS.map((item) => (
            <Link
              key={item.slug}
              to={`/v2/politicas/${item.slug}`}
              className={`v2-aba-botao ${abaAtiva.slug === item.slug ? 'v2-aba-botao--ativa' : ''}`}
            >
              {item.titulo}
            </Link>
          ))}
        ...
        <div className="v2-institucional-conteudo">
          ...
          {abaAtiva.secoes.map((secao) => (
            <div className="v2-institucional-secao" key={secao.titulo}>
              <h3>{secao.titulo}</h3>
...
export default V2PoliticasPage
